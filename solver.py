...
class Model(object):
    def __init__(self, config, vocab, is_train=True, model=None):
        self.config = config
        self.epoch_i = 0
        self.vocab = vocab
    ...
    def build(self, ngpus_per_node):
        ...
        if self.model is None:
            # getattr(models, self.config.model) ---> class 'models.MHRED'
            # 通过models.py文件加载类MHRED，然后实例化类self.model = MHRED(self.config)
        ...
        if self.config.distributed:
            ...
            if self.config.gpu is not None:
                ...
                self.config.batch_size = int(self.config.batch_size / ngpus_per_node)
                self.config.workers = int((self.config.workers + ngpus_per_node - 1) / ngpus_per_node)
        ...
        elif self.config.gpu is not None:
            torch.cuda.set_device(self.config.gpu)
            self.model = self.model.cuda(self.config.gpu)
        else:
    ...
    def train(self, train_sampler, train_data_loader, eval_data_loader):
        epoch_loss_history = []
        for epoch_i in range(self.epoch_i, self.config.n_epoch):
            ...
            for batch_i, (conversations, conversation_length, sentence_length, images, images_length) in enumerate(tqdm(train_data_loader, ncols=80)):
                ...
                input_sentences = to_var(torch.LongTensor(input_sentences), gpu_id=self.config.gpu)
                target_sentences = to_var(torch.LongTensor(target_sentences), gpu_id=self.config.gpu)
                input_sentence_length = to_var(torch.LongTensor(input_sentence_length), gpu_id=self.config.gpu)
                target_sentence_length = to_var(torch.LongTensor(target_sentence_length), gpu_id=self.config.gpu)
                input_conversation_length = to_var(torch.LongTensor(input_conversation_length), gpu_id=self.config.gpu)
                input_images = to_var(torch.stack(input_images), gpu_id=self.config.gpu)
                input_images_length = to_var(torch.LongTensor(input_images_length), gpu_id=self.config.gpu)
            ...
            print('\n<Validation>...')
            self.validation_loss = self.evaluate(eval_data_loader)
    ...
    def evaluate(self, eval_data_loader):
        ...
        batch_loss_history = []
        n_total_words = 0
        for batch_i, (conversations, conversation_length, sentence_length, images, images_length) in enumerate(tqdm(eval_data_loader, ncols=80)):
            # conversations: (batch_size) list of conversations
            #   conversation: list of sentences
            #   sentence: list of tokens
            # conversation_length: list of int
    ...
    def test(self, eval_data_loader):
        self.model.eval()
        batch_loss_history = []
        n_total_words = 0
        for batch_i, (conversations, conversation_length, sentence_length, images, images_length) in enumerate(tqdm(eval_data_loader, ncols=80)):
    ...
    def generate_for_evaluation(self, eval_data_loader):
        self.model.eval()
        n_sample_step = self.config.n_sample_step
        n_sent = 0
